...
function createOfferer(peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name) {
    ...
    if (!localScreenSharing && !remoteScreenSharing) {
        ...
        if (peerUsername === "robot") {
            ...
            // remoteVideo2 = createVideo(peerUsername + '-video2').video2;
            setOnTrack(peer, remoteVideo1.video1);
            setOnTrack(peer, remoteVideo1.video2);
            // Push the new video elements to the array
            mapPeers[peerUsername] = [peer, dc, { video1: remoteVideo1, video2: remoteVideo2 }];
        } else {
            remoteVideo1 = createVideo(peerUsername);
...
function createVideo(peerUsername) {
    ...
    let remoteVideo1, remoteVideo2;

    if (peerUsername === "robot") {
        remoteVideo1 = document.createElement('video');
        ...
        console.log('remoteVideo1:: ', remoteVideo1);
        ...
        // Push the new video elements to the array
        remoteVideos.push({ video1: remoteVideo1, video2: remoteVideo2 });
...
function addLocalTracks(peer, localScreenSharing) {
    if (!localScreenSharing) {
        ...
        localStream2.getTracks().forEach(track => {
            console.log('Adding 2nd localStream tracks.');
