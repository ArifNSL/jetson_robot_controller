    ...
    peer.addEventListener('track', (event) => {
        if (event.streams && event.streams.length > 0) {
            if (remoteVideo1.srcObject === null) {
                ...
                console.log("Set on Track Remote Video 2: ", event.streams[0].id);
            }
            if (event.streams[0].id !== remoteVideo1.srcObject.id) {
                remoteVideo2.srcObject = event.streams[0];
                console.log("Set on Track Remote Video 1: ", event.streams[0].id);
            }
        }
    });
}
